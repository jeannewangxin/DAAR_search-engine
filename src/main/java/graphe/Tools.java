...
public class Tools {
    ...
    public static Map<Integer, HashMap<Integer, Set<Integer>>> floydWarshall2(double[][] grapheJaccard) {
        Map<Integer, HashMap<Integer, Set<Integer>>> paths = new HashMap<Integer, HashMap<Integer, Set<Integer>>>();
        double[][] dist = new double[grapheJaccard.length][grapheJaccard.length];
        ...
        for (int i = 0; i < grapheJaccard.length; i++) {
            for (int j = 0; j < grapheJaccard.length; j++) {
                if (i == j) {
                    ...
                    pa.add(i);
                    HashMap<Integer, Set<Integer>> map1 = paths.containsKey(i) ? paths.get(i)
        ...
        for (int k = 0; k < grapheJaccard.length; k++) {
            for (int i = 0; i < grapheJaccard.length; i++) {
                for (int j = 0; j < grapheJaccard.length; j++) {
                    if (dist[i][j] > dist[i][k] + dist[k][j]) {
                        dist[i][j] = dist[i][k] + dist[k][j];
                        Set<Integer> pa = paths.get(i).get(k);
                        HashMap<Integer, Set<Integer>> map1 = paths.get(i);
                        map1.put(j, pa);
                        paths.put(i, map1);

                    } else if (dist[i][j] == (dist[i][k] + dist[k][j])) {
                        if (i != k & k != j & i != j & dist[i][j] != Double.POSITIVE_INFINITY) {
                            Set<Integer> pa1 = new HashSet<>();
                            pa1.addAll(paths.get(i).get(k));
                            Set<Integer> pa2 = new HashSet<>();
                            pa2.addAll(paths.get(i).get(j));
                            HashMap<Integer, Set<Integer>> map1 = paths.get(i);
                            pa1.addAll(pa2);
                            map1.put(j, pa1);
                            paths.put(i, map1);
    ...
    public static double[] betwennesse_v2(Map<Integer, HashMap<Integer, Set<Integer>>> paths) {
        double betwennesse[] = new double[paths.size()];
        ...
        for (Entry<Integer, HashMap<Integer, Set<Integer>>> e : paths.entrySet()) {
            ...
            for (Entry<Integer, Set<Integer>> f : e.getValue().entrySet()) {
                ...
                for (int path : f.getValue()) {
                    if (path == i || path == j) {
                        continue;
                    }
                    betwennesse[path] += 1.0f / (float) f.getValue().size();
                }
                j++;
            }
            i++;
    ...
    public static double[] betwennesse2(double[][] grapheJaccard) {
        double betwennesse[] = new double[grapheJaccard.length];
        int[][] path = calculShortestPaths(grapheJaccard);
        for (int i = 0; i < grapheJaccard.length; i++) {
            betwennesse[i] = cal(path, grapheJaccard, i);
    ...
    public static double[] pageRank(double[][] grapheJaccard) {
        double[] pageRank = new double[grapheJaccard.length];
        for (int i = 0; i < pageRank.length; i++) {
            pageRank[i] = 0f;
        }
        double sum = 0;
        for (int i = 0; i < grapheJaccard.length; i++) {
            for (int j = (i + 1); j < grapheJaccard.length; j++) {
                if (grapheJaccard[i][j] <= 0.75) {
                    pageRank[i] += 1;
                    pageRank[j] += 1;
                    sum += 2;
