...
public class Automate {
	...
	public ArrayList<Integer> matchString (String s) {
		...
		try {
			ret = parse();
			Automata automata = new Automata(ret);
			result = automata.matchString(s);
		} catch (Exception e) {
	...
	public ArrayList<Point> find (ArrayList<String> text) {
		...
		try {
			...
			for(int i=0;i<text.size();i++) {
				...
				po = automata.matchString(text.get(i));
				for(int y : po) {
...
class RegExTree {
	...
	private String rootToString() {
		if (root == Automate.CONCAT)
			return ".";
		if (root == Automate.ETOILE)
			return "*";
		if (root == Automate.ALTERN)
			return "|";
		if (root == Automate.DOT)
			return ".";
